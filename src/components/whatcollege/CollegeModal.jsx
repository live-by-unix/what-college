...
import { MapPin, TrendingUp, BookOpen, Trophy, Lightbulb, GraduationCap, Award } from "lucide-react";
...
const ratingColors = {
  ...
  "Ultra Reach": "bg-red-50 text-red-700 border-red-200",
};
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="relative h-2.5 bg-slate-100 rounded-full overflow-hidden">
        ...
        />
        <div
          className="absolute top-1/2 -translate-y-1/2 w-3 h-3 bg-blue-600 rounded-full border-2 border-white shadow-sm transition-all duration-500"
...
export default function CollegeModal({ result, open, onClose, apCourses }) {
  if (!result) return null;

  const { college, chance, rating, suggestions, compositeGPA, rigorBonus } = result;
          ...
          <section>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-slate-50 rounded-xl p-3">
                ...
                <p className="text-lg font-bold text-slate-800">{college.difficulty_index}<span className="text-sm text-slate-400">/10</span></p>
              </div>
              <div className="bg-slate-50 rounded-xl p-3">
                <p className="text-xs text-slate-500">Your GPA</p>
                <p className="text-lg font-bold text-slate-800">{compositeGPA}</p>
              </div>
              <div className="bg-slate-50 rounded-xl p-3">
                <p className="text-xs text-slate-500">Rigor Bonus</p>
                <p className="text-lg font-bold text-emerald-600">+{rigorBonus}</p>
              </div>
            </div>
            {studentAPs.length > 0 && (
              <div className="mt-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {studentAPs.map(ap => (
                    <Badge key={ap} variant="outline" className="text-[10px] px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 border-blue-200">
                      {ap}
                    </Badge>
                  ))}
                </div>
              ...
              <div className="mt-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {college.majors.map(m => (
                    <Badge key={m} variant="outline" className="text-[10px] px-2 py-0.5 rounded-full bg-slate-50 text-slate-600 border-slate-200">
                      {m}
                    </Badge>
                  ))}
                </div>
            ...
            <section>
              ...
              <div className="space-y-3">
                {suggestions.map((s, i) => {
                  ...
                  return (
                    <div key={i} className="flex gap-3 p-3 bg-blue-50/50 rounded-xl border border-blue-100/50">
